...
def test_package_json_declares_arktype_2_2_0():
    ...
    assert "arktype" in deps, (
        "arktype is not listed in package.json dependencies."
    )
...
def test_package_json_declares_ark_json_schema():
    package_json = os.path.join(PROJECT_DIR, "package.json")
    with open(package_json) as f:
        data = json.load(f)
    deps = data.get("dependencies", {})
    assert "@ark/json-schema" in deps, (
        "@ark/json-schema is not listed in package.json dependencies."
    )
    assert deps["@ark/json-schema"] == "0.0.4", (
        f"Expected @ark/json-schema version 0.0.4 but got "
        f"{deps['@ark/json-schema']!r}."
    )
...
def test_ark_json_schema_module_installed():
    pkg_dir = os.path.join(
        PROJECT_DIR, "node_modules", "@ark", "json-schema"
    )
    assert os.path.isdir(pkg_dir), (
        f"@ark/json-schema is not installed at {pkg_dir}."
    )
...
def test_ark_json_schema_import_works():
    """Sanity check: @ark/json-schema is importable from Node in the project."""
    result = subprocess.run(
        [
            "node",
            "-e",
            "require('@ark/json-schema'); console.log('ok');",
        ],
        cwd=PROJECT_DIR,
        capture_output=True,
        text=True,
    ...
    assert result.returncode == 0, (
        f"Failed to require('@ark/json-schema') from project: "
        f"stderr={result.stderr}"
    )
    assert "ok" in result.stdout, (
        f"Unexpected stdout when requiring @ark/json-schema: "
        f"{result.stdout!r}"
    )
